...
export const getPlanFromPriceId = (priceId: string | null | undefined): string => {
    if (priceId && priceId === STRIPE_PRICE_RESEARCHER_PRO) return 'researcher_pro';
    if (priceId && priceId === STRIPE_PRICE_RESEARCHER) return 'researcher';
    return 'free';
};
